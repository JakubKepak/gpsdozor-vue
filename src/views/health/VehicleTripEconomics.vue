...
import { Card, Row, Col, Statistic, Skeleton, Table, Tag } from 'ant-design-vue'
...
const tripDataSource = computed(() => (trips.value ?? []).map(t => ({ ...t, key: t.Id })))
</script>

<template>
  <!-- Loading state -->
  <div v-if="isLoading" class="flex flex-col gap-4">
    <h2 class="text-base font-semibold text-gray-900 m-0">{{ t('health.tripEconomics') }}</h2>
    <Row :gutter="[16, 16]">
      <Col v-for="i in 6" :key="i" :xs="12" :sm="8" :lg="4">
  ...
  <!-- Content -->
  <div v-else-if="economics && economics.totalTrips > 0" class="flex flex-col gap-4">
    <h2 class="text-base font-semibold text-gray-900 m-0">{{ t('health.tripEconomics') }}</h2>
    ...
    <Row :gutter="[16, 16]">
      <Col :xs="12" :sm="8" :lg="4">
        <Card :body-style="{ padding: '16px' }">
          <Statistic :title="t('health.totalTrips')" :value="economics.totalTrips" />
        </Card>
      </Col>
      <Col :xs="12" :sm="8" :lg="4">
        <Card :body-style="{ padding: '16px' }">
          <Statistic :title="t('health.colTotalDistance')" :value="economics.totalDistance.toFixed(0)" suffix="km" />
        </Card>
      </Col>
      <Col :xs="12" :sm="8" :lg="4">
        <Card :body-style="{ padding: '16px' }">
    ...
    <!-- Trend charts -->
    <Row v-if="dailyData.length > 1" :gutter="[16, 16]">
      <Col v-if="economics.hasFuelData" :xs="24" :lg="12">
        <LineChartCard
    ...
    <!-- Trip table -->
    <div>
      <h3 class="text-sm font-semibold text-gray-900 m-0 mb-3">{{ t('health.tripList') }}</h3>
      <Table
        :columns="columns"
        :data-source="tripDataSource"
        :pagination="{
          defaultPageSize: 10,
